...
test('perk offers are unique; choice is single-use and fresh runs reset all bonuses',()=>{const s=clean();s.pendingPerks=perkOffer(s.perks,s.contentRandom);assert.equal(new Set(s.pendingPerks).size,3);const id=s.pendingPerks[0];assert.ok(s.choosePerk(id));assert.equal(s.choosePerk(id),false);assert.equal(s.perks.size,1);const next=perkOffer(s.perks,s.contentRandom);assert.ok(!next.includes(id));const r=clean();assert.equal(r.perks.size,0);assert.equal(r.pendingPerks.length,0);assert.equal(r.inventory.capacity,16);assert.equal(Object.keys(PERKS).length,12);});
...
test('health, capacity and economical build perks apply once and charge correct resources',()=>{const s=clean();s.pendingPerks=['tough'];s.choosePerk('tough');assert.equal(s.maxHP,115);assert.equal(s.player.hp,115);s.pendingPerks=['pack'];s.choosePerk('pack');assert.equal(s.inventory.capacity,19);assert.equal(s.choosePerk('pack'),false);s.perks.add('builder');s.perks.add('engineer');assert.equal(s.buildWood,4);assert.equal(s.repairScrap,0);s.builderUsed=true;assert.equal(s.buildWood,6);});
...
test('trunk alarm can be disabled with a timed action; event lifecycle ends without retry duplication',()=>{const s=clean(),trunk=s.facilities.find(f=>f.kind==='trunk')!;Object.assign(s.player,trunk);s.update(1/60,{...idle,interact:true});step(s,2.2);assert.equal(trunk.state,'opened');assert.ok(s.alarmTimer>0);s.update(1/60,{...idle,interact:true});step(s,1.5);assert.equal(s.alarmTimer,0);
 const e=clean();e.eventTimer=0;step(e,.1);assert.ok(e.worldEvent);assert.equal(e.nextEventId,1);step(e,145);assert.equal(e.worldEvent,undefined);const r=clean();assert.equal(r.worldEvent,undefined);assert.equal(r.nextEventId,0);
});
test('seeded runs vary optional caches and equipment without removing opening supplies',()=>{const a=clean(1),b=clean(2),same=clean(1);assert.deepEqual(a.loot,same.loot);assert.notDeepEqual(a.loot.map(l=>l.searched),b.loot.map(l=>l.searched));for(const s of [a,b]){assert.equal(s.ammo,12);assert.equal(s.reserve,60);assert.equal(s.inventory.items.med,1);assert.equal(s.loot[0].searched,false);assert.ok(s.loot.find(l=>l.id==='hospital-case')!.guaranteed!.med!>=2);}});
