...
for(const [i,kind]of (Object.keys(ENEMIES)as EnemyKind[]).entries()){const c=new Character(true,i);c.setKind(kind);c.root.position.set(-9+i*6,0,1);scene.add(c.root);c.animate(1.8,true,false,0);c.wounds({reaction:0,side:1,zone:'TORSO',slow:0,wounds} as Walker);textSign(scene,ENEMIES[kind].name.toUpperCase(),-9+i*6,.1,3.3,4,.5).rotation.x=-Math.PI/2;}
const corpses=new CorpseView(scene);corpses.update((Object.keys(ENEMIES)as EnemyKind[]).map((kind,i)=>({id:i,kind,x:-9+i*6,z:7,age:1.5,fall:0,angle:.2,variant:i%3,wounds})));renderer.render(scene,camera);document.body.dataset.ready='true';
