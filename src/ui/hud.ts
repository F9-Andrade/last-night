...
export class HUD {
  ...
  constructor(){this.root=document.querySelector('#app')!;this.root.innerHTML=layout();this.canvas=this.root.querySelector('#game')!;this.variety=new VarietyHUD(this.root);
    ...
    this.el('map-legend').innerHTML=REGIONS.slice(0,6).map((r,i)=>`<p>${icon(regionIcons[i])}<span>${natural(r.name)}</span></p>`).join('');
  }
  ...
  update(sim:Simulation,dt:number,mouse:{x:number;y:number},project?:(x:number,z:number,y?:number)=>{x:number;y:number},aimTarget=false):void {
    ...
    this.text('objective-title',sim.phase==='dawn'?'Você sobreviveu.':sim.phase==='night'?'Proteja seu abrigo':sim.phase==='day'?'Antes que escureça':'Hora de voltar');this.text('objective-sub',sim.phase==='dawn'?'A recompensa está no depósito.':sim.phase==='night'?'Cuide das três entradas.':sim.phase==='day'?'Encontre suprimentos na cidade.':'Prepare as defesas e a munição.');this.el('objective').classList.toggle('quiet',this.objectiveTime<=0);
    const loot=sim.nearbyLoot,defense=sim.nearbyDefense,action=sim.action;
    const names={search:'Vasculhando',heal:'Aplicando bandagem',build:'Construindo',repair:'Reparando',dismantle:'Desmontando',base:'Reparando abrigo',facility:'Abrindo acesso',silence:'Desligando alarme',event:'Recolhendo reserva'};
    ...
    if(project&&prompt){const at=sim.nearbyFacility??sim.nearbyEvent??loot??defense??sim.player;const p=project(at.x,at.z,1.4);this.style('interaction','left',`${Math.max(180,Math.min(innerWidth-180,p.x))}px`);this.style('interaction','top',`${Math.max(150,Math.min(innerHeight-200,p.y-60))}px`);}
